...
--select * from actor                    act_id act_fname act_lname act_gender
...
--select * from movie_genres             gen_id mov_id

--select * from rating                   mov_id rev_id rev_star num_o_rating 

--select * from reviewer                 rev_id rev_name
...
select rev_name 
		from reviewer	
				where  rev_id = 
						any (select rev_id from rating where rev_stars>=7);
		...
		where act_id 
			in 
				(select act_id from movie_cast where mov_id 
						in (select mov_id from movie where mov_title='Annie Hall'))
...
--------3write a SQL query to find those movies, which released in the country besides UK.
								...
								where mov_id 
									in 
										(select mov_id from rating where rev_id in 
											...
											and 
											act_id 
												in(select act_id from movie_cast where mov_id = movie.mov_id)
...
select mov_title ,mov_id
	 from movie
		 where mov_id not in 
			(select mov_id from rating where mov_id = movie.mov_id)
...
select rev_name 
	from reviewer
			where rev_id 
				in (select rev_id from rating where rev_stars is null)
...
select  rev_name,mov_title,rev_stars
	 from reviewer,movie,rating
			where rev_stars
				in 
				( select rev_stars from rating where mov_id= movie.mov_id and
				 rev_stars!='' and num_o_ratings!=' ' )
				 and 
				 reviewer.rev_id in (select rev_id from rating where mov_id = movie.mov_id)
				 and
				 rev_name in (select rev_name from reviewer where rev_id= rating.rev_id  and reviewer.rev_name!=' ')
...
select rev_name ,mov_title 
	from reviewer , movie
		where 
			rev_id in (select r.rev_id from rating r group by rev_id having  count(r.rev_id) >1  )
...
select mov_title
	from movie
		where mov_id 
			in(select mov_id from rating where rev_id 
	...
	from movie ,rating,reviewer
		where movie.mov_id 
			...
			rev_stars 
				...
				and 
				rev_name in(select rev_name from reviewer where rev_id in (select rev_id from rating where mov_id = movie.mov_id)) 
				group by mov_title,rev_stars,rev_name
...
select mov_title 
	from movie 
		where mov_id 
			in (select mov_id from movie_direction where dir_id 
				in(select dir_id from director where dir_fname ='James' and dir_lname='Cameron'))

---------16 Write a query in SQL to find the name of those movies where one or more actors acted in two or more movies.----------------
select mov_title 
	from movie 
		where mov_id 
			in(select mov_id from movie_cast group by mov_id,act_id having COUNT(mov_id)>=1 and act_id 
				in (select act_id from movie_cast group by act_id having count(act_id)>=2))
...
select dir_fname,dir_lname,count(gen_title) as number_of_generic_movies 
	from director 
		inner join movie_direction
...
----------------------7 write a SQL query to find the movies with year and genres. Return movie title, movie year and generic tit-----------------------------------
...
select concat(dir_fname,' ',dir_lname) as dir_name ,mov_year,gen_title	
	from movie	
		inner join movie_direction
			on movie.mov_id=movie_direction.mov_id
				inner join director
...
select gen_title,avg(mov_time) as avg_mov_time,count(movie_genres.mov_id) as number_of_mov
	from movie	
		inner join movie_genres
		 on movie.mov_id =  movie_genres.mov_id
			inner join genres
...
select mov_title,max(rev_stars) as max_stars
	from movie
		inner join rating 
			on movie.mov_id= rating.mov_id
				where rev_stars>1
